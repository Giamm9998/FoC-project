...
\begin{figure}
    ...
    \begin{bytefield}[bitwidth=0.5em]{64}
        \bitheader[]{64, 56, 48, 40, 32, 24, 16, 8, 0} \\
        \bitbox{8}{Type}
        \bitbox{56}{Payload length} \\
...
\begin{figure}
    ...
    \begin{msc}{}
        ...
        \mess{$upload, E(f, K), Tag(upload \mid\mid seq \mid\mid f, K)$}{client}{server}

        \nextlevel
        \action*{Checks that $f$ can be uploaded}{server}

        \nextlevel[3]
        \mess{$upload\_ans, E(m_1, K), Tag(upload\_ans \mid\mid seq + 1 \mid\mid m_1, K)$}{server}{client}
        ...
        \mess{$upload\_file, E(fc, K), Tag(upload\_file, \mid\mid seq + 2 \mid\mid fc, K)$}{client}{server}
        ...
        \nextlevel[4]
        \mess{$upload\_res, E(m_2, K), Tag(upload\_res \mid\mid seq + 3 \mid\mid m_2, K)$}{server}{client}
        \nextlevel
    \end{msc}
    \centering
    \caption{File upload}
...
\begin{figure}
    ...
    \begin{msc}{}
        ...
        \nextlevel[3]
        \mess{$download, E(f, K), Tag(download \mid\mid seq \mid\mid f, K)$}{client}{server}
        ...
        \nextlevel[5]
        \mess{$download\_ans, E(fc, K), Tag(download\_ans \mid\mid seq + 1 \mid\mid fc, K)$}{server}{client}
...
\begin{figure}
    ...
    \begin{msc}{}
        ...
        \mess{$delete, E(f, K), Tag(delete \mid\mid seq \mid\mid f, K)$}{client}{server}
        ...
        \nextlevel[4]
        \mess{$delete\_conf, E(m_1, K), Tag(delete\_conf \mid\mid seq + 1 \mid\mid m_1, K)$}{server}{client}

        \nextlevel
        \action*{Confirms or aborts deletion}{client}
        \nextlevel[5]

        \mess{$delete\_ans, Tag(delete\_ans \mid\mid seq + 2, K)$}{client}{server}
        ...
        \nextlevel[4]
        \mess{$delete\_res, E(m_2, K), Tag(delete\_res \mid\mid seq + 3 \mid\mid m_2, K)$}{server}{client}
        \nextlevel
    \end{msc}
    \centering
    \caption{File delete}
...
\begin{figure}
    ...
    \begin{msc}{}
        ...
        \nextlevel[3]
        \mess{$list, Tag(list \mid\mid seq, K)$}{client}{server}

        \nextlevel
        \action*{Gets list of files $fl$}{server}

        \nextlevel[3]
        \mess{$list\_ans, E(fl, K), Tag(list\_ans \mid\mid seq + 1 \mid\mid fl, K)$}{server}{client}
...
\begin{figure}
    ...
    \begin{msc}{}
        ...
        \nextlevel[4]
        \mess{$rename, E(m_1, K), Tag(rename \mid\mid seq \mid\mid m_1, K)$}{client}{server}
        ...
        \nextlevel[4]
        \mess{$rename\_ans, E(m_2, K), Tag(rename\_ans \mid\mid seq + 1 \mid\mid m_2, K)$}{server}{client}
...
\begin{figure}
    ...
    \begin{msc}{}
        ...
        \nextlevel[3]
        \mess{$logout, Tag(logout \mid\mid seq, K)$}{client}{server}

        \nextlevel
        \action*{Gracefully closes current session}{server}

        \nextlevel[3]
        \mess{$logout\_ans, Tag(logout\_ans \mid\mid seq + 1, K)$}{server}{client}
...
\begin{figure}
    ...
    \begin{bytefield}[bitwidth=0.5em]{64}
        ...
        \bitbox{56}{Payload length} \\
        \wordbox{1}[bgcolor=lightgreen]{Sequence number} \\
        \wordbox{7}[bgcolor=lightred]{Payload (variable length)}
    \end{bytefield}
